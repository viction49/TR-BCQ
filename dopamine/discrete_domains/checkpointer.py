...
def get_latest_checkpoint_number(base_directory,
  ...
  try:
    checkpoint_files = gfile.Glob(glob)
...
class Checkpointer(object):
  ...
  def __init__(self, base_directory, checkpoint_file_prefix='ckpt',
    ...
    Args:
      ...
      checkpoint_frequency, the frequency at which to checkpoint.

    Raises:
      ValueError: if base_directory is empty, or not creatable.
    """
    if not base_directory:
      print("No path provided to Checkpointer.")
    ...
    try:
      gfile.MakeDirs(base_directory)
      print("successfully created directory {}".format(base_directory))
    except tf.errors.PermissionDeniedError:
      # We catch the PermissionDeniedError and issue a more useful exception.
      print("Unable to create checkpoint path: {}".format(base_directory))
  ...
  def _save_data_to_file(self, data, filename):
    ...
    with gfile.GFile(filename, 'w') as fout:
  ...
  def save_checkpoint(self, iteration_number, data):
    ...
    Args:
      iteration_number, the current iteration number for this checkpoint.
    ...
    print("successfully saved checkpoint dir to {}".format(filename))
  ...
  def _clean_up_old_checkpoints(self, iteration_number):
    ...
    if stale_iteration_number >= 0:
      ...
      stale_sentinel = self._generate_filename(self._sentinel_file_prefix,
                                               stale_iteration_number)
      try:
        gfile.Remove(stale_file)
        gfile.Remove(stale_sentinel)
      except tf.errors.NotFoundError:
