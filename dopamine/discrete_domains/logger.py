...
class Logger(object):
  ...
  def __init__(self, logging_dir):
    ...
    try:
      gfile.MakeDirs(logging_dir)
    except tf.errors.PermissionDeniedError:
      # If it already exists, ignore exception.
      pass
    # if not gfile.Exists(logging_dir):
    #   logging.warning(
  ...
  def log_to_file(self, filename_prefix, iteration_number):
    ...
    with gfile.GFile(log_file, 'w') as fout:
  ...
  def load_from_file(self, filename_prefix, iteration_number):
    ...
    if not self._logging_enabled:
      ...
      return
    log_file = self._generate_filename(filename_prefix, iteration_number)
    with gfile.GFile(log_file, 'rb') as fout:
      data = pickle.load(fout)
    sorted_data = []
    episode_return = []
    for iteration in range(iteration_number):
      key = 'iteration_' + str(iteration)
      values = data[key]
      sorted_data.append(values)
      episode_return.append(sum(values['eval_episode_returns']) / len(values['eval_episode_returns']))
