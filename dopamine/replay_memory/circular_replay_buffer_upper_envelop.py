...
def invalid_range(cursor, replay_capacity, stack_size, update_horizon):
  ...
  Args:
    ...
    replay_capacity, the size of the replay memory.
    stack_size, the size of the stacks returned by the replay memory.
    update_horizon, the agent's update horizon.
  Returns:
...
class OutOfGraphReplayBuffer(object):
  ...
  Attributes:
    add_count, counter of how many transitions have been added (including
  ...
  def __init__(self,
    ...
    Args:
      ...
      replay_capacity, number of transitions to keep in memory.
      batch_size.
      update_horizon, length of update ('n' in n-step update).
      gamma, the discount factor.
      max_sample_attempts, the maximum number of attempts allowed to
        get a sample.
    ...
    self._returns_shape = returns_shape
    self._returns_dtype = returns_dtype
  ...
  def _add_return_storage(self):
    storage_element = ReplayElement('returns', self._returns_shape,
                      self._returns_dtype)
    array_shape = [self._replay_capacity] + list(storage_element.shape)
    self._store[storage_element.name] = np.empty(
      array_shape, dtype=storage_element.type)

  def _add_estimated_return_storage(self):
      storage_element = ReplayElement('estimated_returns', self._returns_shape,
                                      self._returns_dtype)
      array_shape = [self._replay_capacity] + list(storage_element.shape)
      self._store[storage_element.name] = np.empty(
          array_shape, dtype=storage_element.type)
  ...
  def _add_return_storage_bc(self):
    self._store = {}

    storage_element = ReplayElement('action_bc', self._action_shape,
                                    self._action_dtype)
    array_shape = [self._replay_capacity] + list(storage_element.shape)
    self._store[storage_element.name] = np.empty(
      array_shape, dtype=storage_element.type)

    storage_element = ReplayElement('state_bc', self._observation_shape,
                                    self._observation_dtype)
    array_shape = [self._replay_capacity] + list(storage_element.shape)
    self._store[storage_element.name] = np.empty(
      array_shape, dtype=storage_element.type)

  def _add_return_storage_bcq(self):
    self._store = {}
    ...
    storage_element = ReplayElement('reward_bc', self._reward_shape,
                                    self._reward_dtype)
    array_shape = [self._replay_capacity] + list(storage_element.shape)
    self._store[storage_element.name] = np.empty(
        array_shape, dtype=storage_element.type)

    storage_element = ReplayElement('next_state_bc', self._observation_shape,
                                    self._observation_dtype)
    array_shape = [self._replay_capacity] + list(storage_element.shape)
    self._store[storage_element.name] = np.empty(
      array_shape, dtype=storage_element.type)

    storage_element = ReplayElement('terminal_bc', (),
  ...
  def get_range(self, array, start_index, end_index):
    ...
    Args:
      ...
      start_index, index to the start of the range to be returned. Range
  ...
  def is_valid_transition(self, index):
    ...
    Args:
      index, the index to the state in the transition.
  ...
  def is_index_return_larger_than_border(self, index):
      if index >= len(self._store['returns']) or index >= len(self._store['estimated_returns']):
          return False
      returns_index = self._store['returns'][index]
      returns_estimated_index = self._store['estimated_returns'][index]
      ratio = returns_index / returns_estimated_index
      return ratio > self._border

  def _create_batch_arrays(self, batch_size, mode=None, single_iter=False):
    ...
    """
    transition_elements = self.get_transition_elements(batch_size, mode=mode, single_iter=single_iter)
    batch_arrays = []
  ...
  def sample_index_batch(self, batch_size):
    ...
    Args:
      batch_size, number of indices returned.
  ...
  def sample_index_batch_given_range(self, range_list, batch_size):
      """Returns a batch of valid indices sampled uniformly.
      ...
      if self.is_full():
          ...
          min_id = min(
              max(
                  int(self.cursor() - self._replay_capacity + self._stack_size - 1),
                  int(range_list[0])
              ),
              int(self.cursor() - self._update_horizon)
          )
          max_id = max(
              int(self.cursor() - self._replay_capacity + self._stack_size - 1),
              min(
      ...
      while (len(indices) < batch_size and
          ...
          index = np.random.randint(min_id, max_id) % self._replay_capacity
          if self.is_valid_transition(index):
              assert self._border is not None
              if not self.is_index_return_larger_than_border(index):
                  attempt_count += 1
              else:
                  indices.append(index)
          else:
  ...
  def sample_transition_batch(self, batch_size=None, indices=None, mode=None, single_iter=False):
    ...
    Args:
      batch_size, number of transitions returned. If None, the default
        batch_size will be used.
    ...
    if indices is None:
      if mode == 'bcq':
          # fixme: find more rational way
          indices = self.sample_index_batch(batch_size)
      elif mode != 'bc':
          # fixme: find more rational way
          indices = np.random.randint(0, int(self._replay_capacity * 0.8), batch_size)
          indices_test = np.random.randint(int(self._replay_capacity * 0.8), self._replay_capacity - 2, batch_size)
      else:
          # fixme: find more rational way
          indices = self.sample_index_batch_given_range([0, int(len(self._store['action']) * 0.8)], batch_size)
          indices_test = self.sample_index_batch_given_range(
              [
                  int(len(self._store['action']) * 0.8),
                  len(self._store['action']) - 2
              ], batch_size
          )
    assert len(indices) == batch_size

    transition_elements = self.get_transition_elements(batch_size, mode=mode, single_iter=single_iter)
    batch_arrays = self._create_batch_arrays(batch_size, mode=mode, single_iter=single_iter)
      ...
      for batch_element, state_index in enumerate(indices):
        ...
        for element_array, element in zip(batch_arrays, transition_elements):
          ...
          # We assume the other elements are filled in by the subclass.
            ...
            element_array[batch_element] = (
              self._store[element.name][state_index_test])
          # We assume the other elements are filled in by the subclass.

      return batch_arrays

    if mode == 'bc':
        for batch_element, (state_index, state_index_test) in enumerate(zip(indices, indices_test)):
            ...
            # test tuples
            trajectory_indices_test = [(state_index_test + j) % self._replay_capacity
                                       for j in range(self._update_horizon)]
            trajectory_terminals_test = self._store['terminal'][trajectory_indices_test]
            is_terminal_transition_test = trajectory_terminals_test.any()
            if not is_terminal_transition_test:
                trajectory_length_test = self._update_horizon
            else:
                # np.argmax of a bool array returns the index of the first True.
                trajectory_length_test = np.argmax(trajectory_terminals_test.astype(np.bool),
                                                   0) + 1
            next_state_index_test = state_index_test + trajectory_length_test
            trajectory_discount_vector_test = (
                self._cumulative_discount_vector[:trajectory_length_test])
            trajectory_rewards_test = self.get_range(self._store['reward'], state_index_test,
                                                     next_state_index_test)
            ...
            assert len(transition_elements) == len(batch_arrays)
            for element_array, element in zip(batch_arrays, transition_elements):
                if element.name == 'state_bc':
                    element_array[batch_element] = self.get_observation_stack(state_index)
                elif element.name == 'action_bc':
                    element_array[batch_element] = self._store['action'][state_index]
                elif element.name == 'state_bc_test':
                    element_array[batch_element] = self.get_observation_stack(state_index_test)
                elif element.name == 'action_bc_test':
                    element_array[batch_element] = self._store['action'][state_index_test]
                elif element.name in self._store.keys():
                    # Fixme: to be deleted
                    raise NotImplementedError
                    element_array[batch_element] = (
      ...
      for element_array, element in zip(batch_arrays, transition_elements):
        ...
        if element.name == 'state_bc':
          ...
          element_array[batch_element] = (
              self._store[element.name][state_index])
        # We assume the other elements are filled in by the subclass.
        elif element.name == 'state_test':
          element_array[batch_element] = self.get_observation_stack(state_index_test)
        elif element.name == 'state_bc_test':
          element_array[batch_element] = self.get_observation_stack(state_index_test)
        elif element.name == 'reward_test':
          # compute the discounted sum of rewards in the trajectory.
          element_array[batch_element] = np.sum(
            trajectory_discount_vector_test * trajectory_rewards_test, axis=0)
        elif element.name == 'next_state_test':
          element_array[batch_element] = self.get_observation_stack(
            (next_state_index_test) % self._replay_capacity)
        elif element.name == 'next_action_test':
          element_array[batch_element] = (
            self._store['action'][next_state_index_test % self._replay_capacity])
        elif element.name == 'next_reward_test':
          element_array[batch_element] = (
            self._store['reward'][next_state_index_test % self._replay_capacity])
        elif element.name == 'terminal_test':
          element_array[batch_element] = is_terminal_transition
        elif element.name == 'indices_test':
          element_array[batch_element] = state_index_test
        elif element.name == 'returns_test':
          element_array[batch_element] = self._store['returns'][state_index_test]
        elif element.name in self._store.keys():
          # Fixme: to be deleted
          raise NotImplementedError
          element_array[batch_element] = (
  ...
  def get_transition_elements(self, batch_size=None, mode=None, single_iter=False):
    ...
    if mode == 'mc' and single_iter:
      transition_elements = [
        ...
        transition_elements = [
            ...
            ReplayElement('next_reward', (batch_size,) + self._reward_shape,
                          self._reward_dtype),
            ReplayElement('terminal', (batch_size,), self._terminal_dtype),
            ReplayElement('returns', (batch_size,) + self._returns_shape,
                          self._returns_dtype),
            ReplayElement('indices', (batch_size,), np.int32),
        ]
    elif mode == 'bc':
      transition_elements = [
        ReplayElement('state_bc', (batch_size,) + self._state_shape,
                      self._observation_dtype),
        ReplayElement('action_bc', (batch_size,) + self._action_shape,
                      self._action_dtype),
        ReplayElement('state_bc_test', (batch_size,) + self._state_shape,
                      self._observation_dtype),
        ReplayElement('action_bc_test', (batch_size,) + self._action_shape,
                      self._action_dtype),
      ]
    elif mode == 'bcq':
        transition_elements = [
            ...
            ReplayElement('returns', (batch_size,) + self._returns_shape,
                          self._returns_dtype),
            ReplayElement('estimated_returns', (batch_size,) + self._returns_shape,
                          self._returns_dtype),
      ...
      transition_elements = [
          ...
          ReplayElement('returns', (batch_size,) + self._returns_shape,
                        self._returns_dtype),
          ReplayElement('state_test', (batch_size,) + self._state_shape,
                        self._observation_dtype),
          ReplayElement('action_test', (batch_size,) + self._action_shape,
                        self._action_dtype),
          ReplayElement('reward_test', (batch_size,) + self._reward_shape,
                        self._reward_dtype),
          ReplayElement('next_state_test', (batch_size,) + self._state_shape,
                        self._observation_dtype),
          ReplayElement('next_action_test', (batch_size,) + self._action_shape,
                        self._action_dtype),
          ReplayElement('next_reward_test', (batch_size,) + self._reward_shape,
                        self._reward_dtype),
          ReplayElement('terminal_test', (batch_size,), self._terminal_dtype),
          ReplayElement('indices_test', (batch_size,), np.int32),
          ReplayElement('returns_test', (batch_size,) + self._returns_shape,
                        self._returns_dtype),
      ]
  ...
  def _return_checkpointable_elements(self, with_bc=False, train_mode=None):
    ...
    for member_name, member in self.__dict__.items():
      ...
      elif (not member_name.startswith('_')) and (not with_bc):
  ...
  def save(self, checkpoint_dir, iteration_number):
    ...
    for attr in checkpointable_elements:
      ...
      if stale_iteration_number >= 0:
        ...
        try:
          gfile.Remove(stale_filename)
        except tf.errors.NotFoundError:
          pass

  def save_return(self, checkpoint_dir, iteration_number=0):
    """Save the OutOfGraphReplayBuffer attributes into a file.
    ...
    print("Saving returns in {}".format(checkpoint_dir))

    for attr in checkpointable_elements:
      if attr == '$store$_returns':
        filename = self._generate_filename(checkpoint_dir, attr, iteration_number)
        with gfile.GFile(filename, 'wb') as f:
          with gzip.GzipFile(fileobj=f) as outfile:
            # Checkpoint the np arrays in self._store with np.save instead of
  ...
  def save_estimated_return(self, checkpoint_dir, iteration_number=0):
    """Save the OutOfGraphReplayBuffer attributes into a file.
    ...
    for attr in checkpointable_elements:
      if attr == '$store$_estimated_returns':
        filename = self._generate_filename(checkpoint_dir, attr, iteration_number)
        with gfile.GFile(filename, 'wb') as f:
          with gzip.GzipFile(fileobj=f) as outfile:
            # Checkpoint the np arrays in self._store with np.save instead of
  ...
  def save_bc(self, checkpoint_dir, iteration_number=0):
    """Save the OutOfGraphReplayBuffer attributes into a file.
    ...
    for attr in checkpointable_elements:
      if attr in ['$store$_state_bc', '$store$_action_bc', '$store$_reward_bc',
                  '$store$_terminal_bc', '$store$_next_state_bc']:
        filename = self._generate_filename(checkpoint_dir, attr, iteration_number)
        with gfile.GFile(filename, 'wb') as f:
          with gzip.GzipFile(fileobj=f) as outfile:
            # Checkpoint the np arrays in self._store with np.save instead of
  ...
  def load(self, checkpoint_dir, suffix, with_return=False, with_bc=False,
           with_estimated_return=False,
           border=None, train_mode=None):
    """Restores the object from bundle_dictionary and numpy checkpoints.

    Args:
      checkpoint_dir, the directory where to read the numpy checkpointed
        files from.
      suffix, the suffix to use in numpy checkpoint files.
    ...
    # if with_return == True, then add the returns in storage
    assert (with_return and not with_bc) or (with_bc and not with_return) or (not (with_return or with_bc))
    if (with_return and not with_bc) or train_mode == 'BCQ':
      self._add_return_storage()
    if with_estimated_return or train_mode == 'BCQ':
      self._add_estimated_return_storage()
    if with_bc and not with_return:
      self._add_return_storage_bc()

    save_elements = self._return_checkpointable_elements(with_bc=with_bc, train_mode=train_mode)
    ...
    if with_bc:
      perm = np.random.permutation(self._store['action_bc'].shape[0]) * 1
      self._store['action_bc'] = self._store['action_bc'][perm]
      self._store['state_bc'] = self._store['state_bc'][perm, :]
...
class WrappedReplayBuffer(object):
  ...
  def __init__(self,
    ...
    if train_mode == 'BCQ':  # FIXME
        self.create_sampling_ops(use_staging=self.use_staging, mode='bcq')
    else:
        self.create_sampling_ops(use_staging=self.use_staging)
        self.create_sampling_ops(use_staging=self.use_staging, mode='bc')
  ...
  def create_sampling_ops(self, use_staging, mode='mc'):
    ...
    with tf.name_scope('sample_replay'):
      with tf.device('/cpu:*'):
        if mode == 'bc':
            transition_type = self.memory.get_transition_elements(mode='bc')
            transition_tensors = tf.py_func(
                self.memory.sample_transition_batch_bc, [],
            ...
            self.unpack_transition_bc(transition_tensors, transition_type)
        elif mode == 'bcq':
            transition_type = self.memory.get_transition_elements(mode='bcq')
            transition_tensors = tf.py_func(
                self.memory.sample_transition_batch_bcq, [],
                [return_entry.type for return_entry in transition_type],
                name='replay_sample_py_func')
            self._set_transition_shape(transition_tensors, transition_type)
            # Unpack sample transition into member variables.
            self.unpack_transition_bcq(transition_tensors, transition_type)
        else:
  ...
  def unpack_transition(self, transition_tensors, transition_type):
    ...
    self.returns = self.transition['returns']
    self.states_test = self.transition['state_test']
    self.actions_test = self.transition['action_test']
    self.rewards_test = self.transition['reward_test']
    self.next_states_test = self.transition['next_state_test']
    self.next_actions_test = self.transition['next_action_test']
    self.next_rewards_test = self.transition['next_reward_test']
    self.terminals_test = self.transition['terminal_test']
    self.indices_test = self.transition['indices_test']
    self.returns_test = self.transition['returns_test']

  def unpack_transition_bc(self, transition_tensors, transition_type):
    """Unpacks the given transition into member variables.
    ...
    # future versions.
    # behavior cloning
    self.state_bc = self.transition['state_bc']
    self.action_bc = self.transition['action_bc']
    self.state_bc_test = self.transition['state_bc_test']
    self.action_bc_test = self.transition['action_bc_test']

  def unpack_transition_bcq(self, transition_tensors, transition_type):
    """Unpacks the given transition into member variables.
    ...
    self.indices = self.transition['indices']
    self.returns = self.transition['returns']
    self.estimated_returns = self.transition['estimated_returns']
  ...
  def save(self, checkpoint_dir, iteration_number):
    ...
    Args:
      checkpoint_dir, the directory where to read the numpy checkpointed
        files from.
      iteration_number, the iteration_number to use as a suffix in naming
        numpy checkpoint files.
    """
