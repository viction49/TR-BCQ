...
class FixedReplayBuffer(object):
    ...
    def __init__(self, data_dir, replay_suffix, data_set_mode, *args,
        ...
        assert self._data_set_mode in ['ALL', 'POOR', 'HIGH', 'MEDIUM', 'POOR_last_0.6', 'POOR_last_0.8'], \
            "The data set mode only supported in {'ALL', 'POOR', 'HIGH', 'MEDIUM', 'POOR_last_0.6', 'POOR_last_0.8'}"
    ...
    def _load_replay_buffers(self, num_buffers=None):
        ...
        if not self._loaded_buffers:  # pytype: disable=attribute-error
            ...
            ckpt_suffixes = [int(x) for x in ckpt_counters if ckpt_counters[x] >= 6]
            ckpt_suffixes.sort()
            ...
            if num_buffers is not None:
                ...
                elif self._data_set_mode == 'POOR_last_0.6':
                    my_chosen_ckpt_suffixes = ckpt_suffixes[int(num_suffixes * 0.2 * 0.4): int(num_suffixes * 0.2)]
                    ckpt_suffixes = np.random.choice(
                        my_chosen_ckpt_suffixes, num_buffers, replace=False)
                    print("available suffixes", int(num_suffixes * 0.2 * 0.4), int(num_suffixes * 0.2))
                elif self._data_set_mode == 'POOR_last_0.8':
                    my_chosen_ckpt_suffixes = ckpt_suffixes[int(num_suffixes * 0.2 * 0.2): int(num_suffixes * 0.2)]
                    ckpt_suffixes = np.random.choice(
                        my_chosen_ckpt_suffixes, num_buffers, replace=False)
                    print("available suffixes", int(num_suffixes * 0.2 * 0.2), int(num_suffixes * 0.2))
                else:
...
class WrappedFixedReplayBuffer(circular_replay_buffer.WrappedReplayBuffer):
    ...
    def __init__(self,
        ...
        memory = FixedReplayBuffer(
            data_dir, replay_suffix, data_set_mode,
